...
//  }
  ...
  private SegmentsContent segmentBean;
...
//  private static final int MAX_OPTIMIZE_LOOPS = 3;
  ...
  public NutchBean(Configuration conf, Path dir) throws IOException {
    ...
    String url=conf.get("solr.server.url", "http://localhost:8983/solr/collection1");//"http://localhost:8983/solr/collection2";
    searchBean = new SolrBean(conf,  url);

    segmentBean = new SegmentsContent(conf, new Path(dir, "segments"));
