...
public class NewGenerator extends Configured implements Tool {
  ...
  private static class PreSelector implements
  ...
  /// 从主机信息库中查找IP
  private static class Finder implements
      Mapper<Text, CrawlDatum, Text, CrawlDatum>,
    ...
    public void reduce(Text host, Iterator<CrawlDatum> values,
      ...
      while (values.hasNext()) {
        ...
        if (IPUtils.hasIP(datum)) { // 来自主机信息库
          try{
            String ipStr = IPUtils.getIPString(datum);
            if(ipStr != null){
              ipFound = true;
              textIP = new Text(ipStr);
            }
          }
...
//        entry.getMetaData().put(Nutch.WRITABLE_IP_KEY, textIP);
        IPUtils.setIP(entry, textIP.toString());
  ...
  public static class Selector implements
    ...
    public void map(Text key, CrawlDatum value,
      ...
      // // consider only entries with a retry (or fetch) interval lower than
      // threshold
      // if (intervalThreshold != -1 && crawlDatum.getFetchInterval() >
      // intervalThreshold) return;
  ...
  public Path[] generate(Path dbDir, Path hostDir, Path segments, int numLists,
    ...
    return generate(dbDir, hostDir, segments, numLists, topN, curTime, filter,
        normalise, false, 1);
  }
  ...
  public Path[] generate(Path dbDir, Path hostDir, Path segments, int numLists,
    ...
    FileInputFormat.addInputPath(jobPreselect, new Path(dbDir,
    ...
    LOG.info("Generator: Retrieving IP from hostdb.");

    Path tempDir2 = new Path(getConf().get("mapred.temp.dir", ".")
        + "/generate-temp-" + java.util.UUID.randomUUID().toString());

    JobConf jobFinder = new NutchJob(getConf());
    jobFinder.setJobName("generate: match from " + hostDir);

    FileInputFormat.addInputPath(jobFinder, new Path(hostDir,
        HostDb.CURRENT_NAME));
    ...
    tempDir = new Path(getConf().get("mapred.temp.dir", ".")
        + "/generate-temp-" + java.util.UUID.randomUUID().toString());
  ...
  public int run(String[] args) throws Exception {
    if (args.length < 3) {
      System.out
          .println("Usage: NewGenerator <crawldb> <hostdb> <segments_dir> [-force] [-topN N] [-numFetchers numFetchers] [-adddays numDays] [-noFilter] [-noNorm][-maxNumSegments num]");
      return -1;
    }

    Path dbDir = new Path(args[0]);
    Path hostDir = new Path(args[1]);
    ...
    try {
      Path[] segs = generate(dbDir, hostDir, segmentsDir, numFetchers, topN,
