...
//  private String mode = PARTITION_MODE_IP;
...
//      LOG.error("Unknown partition mode : " + mode + " - forcing to byIP");
//      mode = PARTITION_MODE_IP;
    }
  ...
  public int getPartition(Text key, Writable value, int numReduceTasks) {
    ...
    } else if (mode.equals(PARTITION_MODE_IP)) {// 根据IP来进行partition，从而实现generator的礼貌性
      try {
        InetAddress address = IPUtils.getIP((CrawlDatum) value);
        if (address == null) {
          address = InetAddress.getByName(url.getHost());
        }
        hashCode = address.getHostAddress().hashCode();
      } catch (UnknownHostException e) {
        Generator.LOG.info("Couldn't find IP for url: " + url);
      }
    }
